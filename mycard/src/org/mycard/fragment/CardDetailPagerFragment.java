...
public class CardDetailPagerFragment extends BaseFragment implements LoaderCallbacks<Cursor>, IDataObserver{
	...
	**/
	private View initView(LayoutInflater inflater) {
		...
		Bitmap cardImage = Model.peekInstance().getBitmap(mImageItem, Constants.IMAGE_TYPE_ORIGINAL);
		if (cardImage != null) {
			mImageItemController.setBitmap(cardImage);
		} else {
			Bitmap thumbnail = Model.peekInstance().getBitmap(mImageItem, Constants.IMAGE_TYPE_THUMNAIL);
			if (thumbnail != null) {
	...
	public Loader<Cursor> onCreateLoader(int arg0, Bundle arg1) {
		return  new CursorLoader(mActivity, YGOCards.Texts.CONTENT_URI, new String[]{YGOCards.Texts.DESC},
