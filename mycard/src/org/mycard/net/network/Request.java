...
class Request {
    ...
    private void setReceivedContentRange(Map<String, String> headers) {
    	String range = null;
		if (headers == null || TextUtils.isEmpty(range = headers.get("Range")))
			return;
		...
		if (space > 0 && dash > 0 && dash > space) {
			try {
				mReceivedBytes = Integer.parseInt(range.substring(space + 1, dash));
